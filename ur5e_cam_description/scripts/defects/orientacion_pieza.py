            ...
            if (suma == 0):
                ...
                if (c[0][0] - c[elem_horiz][0] < 0):
                    for x in range (c[0][0], c[elem_horiz][0]):
                        ...
                        recta_prof_hist.append([img_prof[y,x,0], img_prof[y,x,1], img_prof[y,x,2]])
                ...
                height, width, tf = img_prof_esquinas.shape
                esquinas = []

                # Me fijo en la imagen sacada de canny y selecciono los bordes
                ...
                plt.figure()
                plt.plot(np.array(recta_prof_hist)[50:300])
                plt.show()
                cv.waitKey(0)
                cv.destroyAllWindows()
